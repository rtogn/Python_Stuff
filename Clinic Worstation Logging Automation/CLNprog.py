...
DL_Blank = "./Documents to Print/template_DL.pdf"
...
E4 = ttk.Combobox(root, state='readonly')
E4['values']=doclist
pat = patient()
def printboxes():
...
if dl_input:
    ...
    # pdfcreate. run requires: lastname, firstname, dob, dos, doctor, dl_list, out_list, kit_list, template_req, file_tag
    pdfcreate.run(lastname, firstname, dob, date, doctor, str(dl_input), str(out_input), str(kit_input), account.blankreq, f'{date}DL')
